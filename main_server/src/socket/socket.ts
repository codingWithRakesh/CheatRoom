...
io.on("connection", (socket: Socket) => {
    ...
    socket.on("join-room", ({ userId, roomCode }: { userId: string; roomCode: string }) => {
        socket.join(userId);
    ...
    socket.on("stop_typing", async ({ room, visitorId }: { room: string; visitorId: string }) => {
        // const codeHash = CryptoUtils.hashRoomCode(room);
        // const roomD = await Room.findOne({ codeHash });
    ...
    socket.on("exit-room", async ({ roomCode, userId }: { roomCode: string; userId: string }) => {
        if (!roomCode || !userId) {
